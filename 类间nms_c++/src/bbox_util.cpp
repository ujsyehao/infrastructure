...
template <typename Dtype>
void ApplyNMSFast(const Dtype* bboxes, const Dtype* scores, const int num, const int num_class, 
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, map<int, vector<int> >* indices) {
  ...
  vector<int> a;
  vector<int>* tmp_indice = &a;
  tmp_indice->clear();

  int valid_class = num_class - 1;	

  // please refer to SSD source code note
  //std::cout << "score size: " << score_index_vec.size() << std::endl;

  while (score_index_vec.size() != 0) {
    int idx = score_index_vec.front().second;
    int box_idx = 0;
	//std::cout << "idx: " << idx << std::endl;
	if (idx > num / valid_class) {
		//std::cout << "pre-idx: " << idx << std::endl;
		//box_idx = idx - num / 2;
		box_idx = idx % (num / valid_class);
		//std::cout << "post-idx: " << box_idx << std::endl;;
	} else {
		box_idx = idx;
	}
	...
	for (int k = 0; k < tmp_indice->size(); ++k) {
	  if (keep) {
	    const int kept_idx = (*tmp_indice)[k];
		int keep_box_idx = 0;
		if (kept_idx > num / valid_class) {
			keep_box_idx = kept_idx % (num / valid_class);
		} else {
			keep_box_idx = kept_idx;
		}
	    float overlap = JaccardOverlap<Dtype>(bboxes + box_idx * 4, bboxes + keep_box_idx * 4);
	...
	if (keep) {	  	
	  tmp_indice->push_back(idx);
	}
	
    score_index_vec.erase(score_index_vec.begin());
  }
  
  std::cout << "keep size: " << (*tmp_indice).size() << std::endl;
  vector<int> class1;
  vector<int> class2;
  for (int i = 0; i < tmp_indice->size(); i++) {
	int keep_idx = tmp_indice->at(i);
	//std::cout << keep_idx << " ";
	if (keep_idx > num / valid_class) {
		//std::cout << " " << keep_idx;
		class2.push_back(keep_idx);
	} else {
		class1.push_back(keep_idx);
	}
  }
  (*indices)[1] = class1;
  (*indices)[2] = class2;
...
template
void ApplyNMSFast(const float* bboxes, const float* scores, const int num, const int num_class, 
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, map<int, vector<int> >* indices);
